...
| 10 | **Peak lifecycle — durability of per-peak state** | 🟡 In progress. Swift Phases 0–6b committed + verified; **Phase 7 + analysis-range-setting removal + a TapToneAnalyzer deinit crash-fix = run-reviewed, UNCOMMITTED (awaiting commit)**; **next = Phase 8 validation + doc deliverables, then Phase 9 ports**. Ports (Python, then web) gated on Phase 8. | [PEAK-LIFECYCLE-PLAN-SWIFT.md](PEAK-LIFECYCLE-PLAN-SWIFT.md) |
