...
|---|---|---|---|
| 1 | **Task 3 / 6-TEST** — cross-platform test-suite normalization (major) | **In progress.** Phases 1-2 done; Phase-3 PC-1..4 done (2026-07-09); **3c consolidation ✅ complete 2026-07-12** (web analyzer/device now mirror canonical `TapToneAnalyzer`/`RealtimeFFTAnalyzer`); EG-1/EG-2/EG-3 + frozen-peak-recalc (engine half, 3-way) done. **NEXT: pure-gap suites** (guitar annotation-state, import-persistence) → orphan test back-ports → PC-1 docs → P3 (also completes frozen-peak-recalc's selection/annotation half). | [6-TEST-NORMALIZATION.md](6-TEST-NORMALIZATION.md) |
