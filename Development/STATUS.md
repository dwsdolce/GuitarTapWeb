...
|---|---|---|---|
| 1 | **Task 3 / 6-TEST** — cross-platform test-suite normalization (major) | **In progress.** Phases 1-2 done; Phase-3 PC-1..4 done (2026-07-09); **3c consolidation ✅ complete 2026-07-12** (web analyzer/device now mirror canonical `TapToneAnalyzer`/`RealtimeFFTAnalyzer`); EG-1/EG-2/EG-3 + frozen-peak-recalc (engine half) + import-persistence — all 3-way. `annotation-state` is entirely the selection/override/offset subsystem → **rescheduled under P3** (it's view-side today; testing it now would be thrown away when P3 moves it to the analyzer). PC-1 docs ✅ (Cancel-as-restart + New-Tap-only-when-complete in all 3 HelpViews + shared manual) — **Phase 3 complete**. **NEXT: Phase 4 — orphan test back-ports** (Swift/Python suites for the 4 web-only slugs: guitar-fft, gated-capture, status-message, tap-count-change). **P3** (item 2 restructure) then lands frozen-peak-recalc's selection half + annotation-state 3-way. | [6-TEST-NORMALIZATION.md](6-TEST-NORMALIZATION.md) |
