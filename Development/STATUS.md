...
|---|---|---|---|
| 1 | **Task 3 / 6-TEST** — cross-platform test-suite normalization (major) | **In progress** — Phases 1-2 done; **Phase 3: PC-1..PC-4 ALL done + validated + committed** (2026-07-09). PC-1 Cancel=restart + button rule (all 3) + web fallout (arming, status, taps-lock); PC-2 web `statusMessage.ts` extraction; PC-3 Python brace "Tap X/N"; PC-4 web `setConfig` progress re-fire (Python/Swift already correct). **3c consolidation UNDERWAY** (spec [TAPTONEANALYZER-CONSOLIDATION.md](TAPTONEANALYZER-CONSOLIDATION.md); goal = align web arch to canonical, names+responsibilities): 3c-0 rename + 3c-A count-facts ✅ committed 2026-07-10; **next = 3c-A2** (completion/detection facts) → 3c-B material → 3c-C device split (AudioEngine→RealtimeFFTAnalyzer, folds in EG-1) → 3c-D collapse rules. Then pure gaps + orphan back-ports; PC-1 docs + EG-2 pending | [6-TEST-NORMALIZATION.md](6-TEST-NORMALIZATION.md) (plan + PC-1..4 + EG) · [TAPTONEANALYZER-CONSOLIDATION.md](TAPTONEANALYZER-CONSOLIDATION.md) |
