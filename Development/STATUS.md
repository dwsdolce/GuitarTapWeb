...
| 10 | **Dot list vs Annotation list — web `isKnown` divergence + no dot-list parity test** | 📋 Open (tackle next). **(1) Hidden web bug:** Swift (`allPeaksInRange`) + Python (`_on_peaks_changed_scatter`) filter the chart **dots** by `isKnown(frequency)`; the web filters by **assigned mode** (`modeByPeak`) and never uses `isKnown`. Identical in the auto path (so the shipped render fix is fine), but diverges under **user mode overrides** — the web dot candidate set must move off `displayPeaks` onto `isKnown ∩ range` (a marker-building change; panel stays assigned-mode). **(2) Test structure:** pin BOTH the dot list (`allPeaksInRange` rule — NO parity coverage today) and the annotation list (`visiblePeaks` — partial via `test/annotation-state`) as pure functions on all 3, oracle fixture incl. override + out-of-band cases, cross-platform identity. The wedge for the view-layer parity item. | [DOT-ANNOTATION-PARITY.md](DOT-ANNOTATION-PARITY.md) |
| 11 | **Python: progress bar lingers after load-guitar-following-material** | ⏸ PARKED 2026-07-17 (user) — may just be reset-timing differing across platforms, not a real bug. **Confirm it reproduces before acting.** Progress bar = `_sb_progress` (`tap_tone_analysis_view.py`), visibility at `:2358`/`:3276`. | *(this row)* |
...
| [DOT-ANNOTATION-PARITY.md](DOT-ANNOTATION-PARITY.md) | Dot vs annotation lists | web dots filter by assigned-mode not `isKnown` (Swift/Python) → override-only hidden bug; + a 3-platform dot/annotation parity-test structure |
