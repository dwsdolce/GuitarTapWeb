...
|---|---|---|---|
| 1 | **File paths & names audit → 1.0.2 blocker** | ⏳ IN PROGRESS. Release testing found inconsistencies in where files save, what they're named, the required-name rule, and the WAV folder/sandbox story. **All of it is in 1.0.2** (incl. the Chromium `showDirectoryPicker`). 8 ordered steps; **Steps 0–5 committed + run-reviewed** (spike proved security-scoped bookmarks → §4b stands; text corrections; filename-stem consolidation; Measurement Name required; bounded pre-roll; macOS export-dialog now defaults to the real `~/Documents/GuitarTap` via `getpwuid`, verified). **Step 6 ✅ CODE + DOCS DONE, NOT YET COMMITTED — awaiting user run-review** (the real gate: picker + bookmark round-trip can't be unit-tested; steps in spec §Step-6). New `WavDumpFolder` (Swift) / `wav_dump_folder.py` (Python), same interface; macOS security-scoped bookmark, Python plain path + `QStandardPaths` (fixes the hardcoded `~/Documents`); Settings folder row (path + Show-in-Finder/Open + Change… + Use Default); arm-time reachability dialog at New Tap (Change Location / Turn Off Saving / Cancel); web stays Downloads-only (picker REVERSED — re-grant-per-load unusable), caption drops "(diagnostic)". `@parity model/wav-dump-folder` + `test/wav-dump-folder` (69 groups). Suites Swift 371 · Python 477 · web 247. **⚠ ALSO uncommitted: the Step-1/3 doc catch-up** (ch07, ch08, HelpView, help_view.py, QuickStartGuide.tsx) + STATUS/spec — all fold into Step 6's commit. **NEXT = Step 7** (incidentals §7: Swift fractional-second JSON filename [may already be done in Step 2], Python `~/Desktop/guitar_tap_raw_capture.wav` debug write) then Step 8 (release notes). Path field + Show-in-Finder + Change… on all 3; default = the app's `Documents/GuitarTap` (= the CONTAINER on sandboxed platforms, so NO first-run prompt — only Change… needs a bookmark); stale-folder = error+re-pick, `isStale` = re-mint silently; Python default from `QStandardPaths`; web Chromium `showDirectoryPicker` (in 1.0.2), Safari/Firefox stay Downloads. Debug log does NOT follow. | [FILE-PATHS-AND-NAMES-SPEC.md](FILE-PATHS-AND-NAMES-SPEC.md) |
| 2 | **1.0.2 release** | Notes written + committed (Swift 391 · Python 434 · web 105), user run-reviewed. **Held: item 1 must land first** (it's part of this release). Notes will need extending for the item-1 changes and the files RENAMED to the new build number. Notes: Swift `Documentation/ReleaseNotes-1.0.2-<build>.md` + PDF · Python `docs/…` + PDF · web in-app `src/components/ReleaseNotes.tsx` (`@parity none`). **Build number = commit count AT the release commit.** Swift/Python notes cumulative (prepend, rename, keep prior history byte-identical). | [FILE-PATHS-AND-NAMES-SPEC.md](FILE-PATHS-AND-NAMES-SPEC.md) §9c |
