...
|---|---|---|---|
| 1 | **Task 3 / 6-TEST** — cross-platform test-suite normalization (major) | **In progress** — Phases 1-2 done; **Phase 3: PC-1..PC-4 ALL done + validated + committed** (2026-07-09). PC-1 Cancel=restart + button rule (all 3) + web fallout (arming, status, taps-lock); PC-2 web `statusMessage.ts` extraction; PC-3 Python brace "Tap X/N"; PC-4 web `setConfig` progress re-fire (Python/Swift already correct). **Next: `TapSession` state-ownership consolidation** (wire tapSession.ts into App via useSyncExternalStore) → remaining pure gaps + orphan back-ports; PC-1 docs + EG-1 pending | [6-TEST-NORMALIZATION.md](6-TEST-NORMALIZATION.md) (plan + PC-1..4 + EG-1) · [PHASE6-PARITY.md](PHASE6-PARITY.md) § 6-TEST |
