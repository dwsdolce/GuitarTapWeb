...
|---|---|---|---|
| 1 | **File paths & names audit → 1.0.2 blocker** | ⏳ IN PROGRESS. Release testing found inconsistencies in where files save, what they're named, the required-name rule, and the WAV folder/sandbox story. **All of it is in 1.0.2** (incl. the Chromium `showDirectoryPicker`). 8 ordered steps; **Steps 0–3 committed + Step 3 user run-reviewed** (spike proved security-scoped bookmarks → §4b stands; text corrections; filename-stem consolidation; Measurement Name now required — model predicate `isValidName`/`normalizedName`, all Save buttons bind to it). **NEXT = Step 4** (bounded pre-roll — keep ~2 s before the first tap; bounds the WAV, preserves the ≥0.5 s playback lead-in, makes every dump a valid fixture; pure model, no UI). | [FILE-PATHS-AND-NAMES-SPEC.md](FILE-PATHS-AND-NAMES-SPEC.md) |
