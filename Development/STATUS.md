...
| 4 | **Platform parity gaps** (Swift/Python/web behavior + UI) | Backlog — **4 open** (OUT-2/3 from the 6-3c review; **OUT-4** material tap-detection model — relative noise-floor EMA vs absolute dBFS; **OUT-5** reduce-count-mid-sequence — Swift defers vs Python synchronous finalise). **OUT-1 ✅ FIXED** — the status state-machine alignment made the Swift/Python warm-up silent, so phase-guidance is now visible (Swift+Python source refactored + reveal tests; web was already conformant). A **separate cross-platform effort, after Phase 6**; rest not started. | [PLATFORM-PARITY-GAPS.md](PLATFORM-PARITY-GAPS.md) · [STATUS-STATE-MACHINE.md](STATUS-STATE-MACHINE.md) |
