...
|---|---|---|---|
| 1 | **File paths & names audit → 1.0.2 blocker** | ⏳ IN PROGRESS. Release testing found inconsistencies in where files save, what they're named, the required-name rule, and the WAV folder/sandbox story. **All of it is in 1.0.2** (incl. the Chromium `showDirectoryPicker`). 8 ordered steps; **Steps 0–4 committed + run-reviewed** (spike proved security-scoped bookmarks → §4b stands; text corrections; filename-stem consolidation; Measurement Name required; bounded pre-roll — session WAV keeps ~2 s before the first tap then records live, verified 8 s idle → 3.17 s file). Web session buffer normalized to flat samples = structurally identical to Swift/Python. **NEXT = Step 5** (macOS export-dialog default — `defaultSaveDirectory` resolves the real home via `getpwuid`, not the sandbox container; de-dup the 2nd copy in `PlatformAdapters.swift`; Swift-only). | [FILE-PATHS-AND-NAMES-SPEC.md](FILE-PATHS-AND-NAMES-SPEC.md) |
