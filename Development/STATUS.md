...
| 10 | **Peak lifecycle — durability of per-peak state** | 🟡 In progress. Swift Phases 0–7 + doc deliverables DONE (user-reviewed; uncommitted). **Phase 9 Python port: Phases 1 & 2 DONE + user-verified** (suite 532, parity 79; incl. an annotation Reset-Position parity fix); web after Python, Phase 3 next. A `TapToneAnalyzer.deinit` crash under swift-testing **parallelism** is **PARKED** — serializing suites did NOT fix it (it's a cross-suite teardown race); **not a production path** (one analyzer, deinit at quit). | [SWIFT plan](PEAK-LIFECYCLE-PLAN-SWIFT.md) · [PYTHON plan](PEAK-LIFECYCLE-PLAN-PYTHON.md) · [deinit crash](DEINIT-CRASH-INVESTIGATION.md) |
