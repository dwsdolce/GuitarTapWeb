...
|---|---|---|---|
| 1 | **File paths & names audit → 1.0.2 blocker** | ⏳ IN PROGRESS. Release testing found inconsistencies in where files save, what they're named, the required-name rule, and the WAV folder/sandbox story. **All of it is in 1.0.2** (incl. the Chromium `showDirectoryPicker`). 8 ordered steps; **Steps 0–2 committed** (spike proved security-scoped bookmarks work → §4b stands; text corrections; filename-stem consolidation). **NEXT = Step 3** (Measurement Name required — `canSaveMeasurement` on the MODEL, not view `disabled:`; loaded-measurement pre-fill; persistence-suite fallout). | [FILE-PATHS-AND-NAMES-SPEC.md](FILE-PATHS-AND-NAMES-SPEC.md) |
