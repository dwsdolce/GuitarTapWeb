...
_Last updated: 2026-07-19 — **1.0.2 RESPIN at Step 8 (ship)**, all holds off (user). Master plan =
...
| 2 | **Peak finding emits duplicate peaks** (CORE, all 3) | ⏳ **CODE WRITTEN + ALL SUITES GREEN — NOT USER-VERIFIED.** Root cause: `findPeaks` interleaved detection with classification, so an overlapping-mode bin minted two peaks and the assembly reconciled by id — every guitar capture on every platform saved one duplicate. **Fixed all 3:** `findPeaks` is now a single mode-blind sweep (Swift 211→77 lines), classification stays in `classifyAll`; duplicates in existing files are **healed at decode** (incl. `tapEntries`) with the library force-saved. Also fixed: Python's card layer used frequency as identity in 3 places (`freq_index` / `_card_for_freq` / `_auto_mode_map`); Python `test/peaks` was mis-tagged and tested reimplementations inside the test file; a pre-existing **flamenco band inversion**; the false "bands are non-overlapping" doc comment. **Suites: Swift 403 · Python 522 · web 305, all pass.** Parity `--check` clean, 77 groups. **NEXT = user run-review (step 9); NOTHING COMMITTED until it passes.** | [PEAK-FINDING-DUPLICATE-PEAKS.md](PEAK-FINDING-DUPLICATE-PEAKS.md) |
| 3 | **Architectural-parity restructure** (view layer) | 📋 Write the **spec first** to size it; no code yet. Lands the 2 parked test items (frozen-peak-recalc selection half + `annotation-state` 3-way). | [RESTRUCTURE-NOTES.md](RESTRUCTURE-NOTES.md) |
...
| 5 | **Swift audio buffer size** (4800 frames, not the 1024 requested) | 🔶 NOT blocking (immaterial to lutherie). One cause → ring-out/peak-freq/WAV-length divergence. The 3-tap plate FALSIFIED the "first-tap only" narrowing → it's **per-tap**. **NEXT = iPad `buffer.frameLength` experiment** (may confirm root cause outright); do fix A first. | [AUDIO-BUFFER-SIZE.md](AUDIO-BUFFER-SIZE.md) · MATERIAL-MULTITAP §3 |
| 6 | **Audio watchdog blind to a SILENT stream** | 🔶 NOT blocking (self-recovers). "Alive" = chunks arriving, not chunks *containing audio*. Fix needs a signal-level criterion + design (false-positive risk). | [AUDIO-WATCHDOG-SILENT-STREAM.md](AUDIO-WATCHDOG-SILENT-STREAM.md) |
| 7 | **Project hub repo** (docs + cross-repo bug tracking) | 📋 IDEA, needs a spec; no action yet. Enabler = Swift going open source. | [PROJECT-HUB-REPO.md](PROJECT-HUB-REPO.md) |
| 8 | **Material multi-tap: Swift ~2 dB / sub-bin below Python/web** | 🔶 NOT blocking (fL agrees; derived props follow frequency). **Same 4800-buffer cause as the Swift audio-buffer-size item** — resolve there, don't chase separately. | MATERIAL-MULTITAP §3 |
| 9 | **Results panel cross-platform consistency** | 📋 Post-release; presentation only (numbers correct). 7 divergences pointing 3 ways (order · Showing-row · nesting · boxing · labels · title-colour · spacing) — this **includes** the Python Gore-box **nesting** (§3) and vertical **spacing** (§7) once mis-filed as separate "P/Q" items. Overall-Quality colour already fixed. Spec against Swift first. | [RESULTS-PANEL-CONSISTENCY.md](RESULTS-PANEL-CONSISTENCY.md) |
| 10 | **Peak Min is documented as a display filter but is a detection gate** | 📋 Open — found in Step-8 run-review 2026-07-19. Peak Min is `effectiveThreshold` in `findPeaks`: peaks below it are **never detected, so never saved**; the file holds only peaks above the capture-time value. `showUnknownModes` is display-only. The **manual says the opposite in 3 places** (§2.7, §8, and §3.6 which understates the wand — it *re-detects* from the saved spectrum, the only route to those peaks). Concrete: the iPhone fixture captured at −60 holds 8 peaks / 0 Unknown; the same recording replayed at −70 gives 36 / 5. Not caused by the duplicate-peak fix (proven across a 25 dB sweep). **Design question open — what should the slider do on a loaded measurement, where it can only subtract?** (The All/Selected/None report was resolved as not-a-defect — the default 75–350 Hz display range filters chart and panel together.) | [PEAK-MIN-SEMANTICS.md](PEAK-MIN-SEMANTICS.md) |
| 11 | **Replay does not bit-reproduce live capture** | 📋 Open — found in the peak-finding run-review automation 2026-07-19. Same-platform WAV replay reproduces the saved measurement *closely but not bit-for-bit*: replay-vs-replay is deterministic (0.000 dB) and resonances reproduce to ≤0.003 dB, but **every spectrum bin differs by ~0.02 dB mean** (0/3702 exact in the ≥−100 dB range) — the gated FFT window lands on a slightly different sample range live-vs-replay. **PROVEN not caused by the peak-finding fix** (replayed spectrum byte-identical with/without the change on all 3, via stash-and-fingerprint). Requirement (user): replay must exactly reproduce live capture. Likely same window-alignment family as the **Material multi-tap / Swift audio-buffer** items — **confirm before merging; MATERIAL-MULTITAP doc left untouched per user.** Re-run harness: `Development/playback-validation/`. | [PLAYBACK-BIT-IDENTITY.md](PLAYBACK-BIT-IDENTITY.md) |
...
| [PEAK-MIN-SEMANTICS.md](PEAK-MIN-SEMANTICS.md) | Peak Min semantics | Peak Min gates detection, not display; manual wrong in 3 places; open design question for the loaded-measurement slider |
