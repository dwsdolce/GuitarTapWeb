...
| 9 | **Mic-name matching warns on cross-platform files** (all 3, surfaced on Windows) | ⏳ **DECIDED 2026-07-19 — implementing.** **Decision: stop trying to prove device identity; make the warning honest.** (a) Delete the web's parenthetical-stripping in `normMic` (it IS the Windows bug and can collapse two different mics onto "microphone"); keep **safe normalisation only** — trim/collapse-whitespace/lowercase. (b) **Reword the no-match case on all 3** to state uncertainty + impact. NATIVES (they enumerate, so "no connected device matches"): *"Recorded with 'A'. No connected microphone matches that name — it may be unplugged, or attached under a different name on this platform. Peak frequencies should still be comparable; input levels, the tap threshold, and faint peaks (such as FLC) may differ."* WEB (knows the selected input): *"Recorded with 'A'; currently using 'B'. Guitar Tap can't tell whether these are the same microphone. Peak frequencies should be comparable; input levels, the tap threshold, and faint peaks (such as FLC) may differ."* (c) **REJECTED: auto-select-on-load for the web** — the web already auto-selects on ATTACH (user-verified), so the right mic is already selected; load-time matching would fail cross-platform and falsely claim "not connected". **Rationale:** none of the 3 can distinguish *genuinely unplugged* from *present under a different name* — Swift loading a Python/PC file hits the same false positive with a stronger (worse) sentence. Impact is mild by design: frequencies are mic-independent; magnitudes, peak SELECTION in marginal ranges, faint-peak (FLC) detection and trigger levels are not. **Original diagnosis below.** 📋 Found 2026-07-19 in Step-8 testing. Loading a **Swift-saved** measurement in **Windows Chrome** always shows the mic warning. Web cause: `normMic` (`fromLive.ts`) strips **all** parentheticals, so Chrome's `Microphone (Umik-1  Gain: 18dB)` collapses to `microphone` while the stored `Umik-1  Gain: 18dB` → `umik-1 gain: 18db` — never equal. **NOT web-only:** Swift/Python compare raw `==` with no normalisation, so the same physical UMIK-1 stored by Swift (`Umik-1  Gain: 18dB`) vs Python/PC (`Microphone (…)`) **already warns 3-ways** (see the brace-umik-1 fixtures). Swift matches **UID-first** (stable CoreAudio UID), name only as fallback; browsers have no equivalent (deviceIds rotate per session/origin), so the web is structurally stuck on the name leg — a forced platform adaptation, not a gratuitous divergence. **DECISION NEEDED:** (a) canonicalise the mic name at **save** time (fixes all three), (b) web-side *unwrap* heuristic (strip a trailing `(vid:pid)`, unwrap a generic `Microphone (…)` wrapper — never collapse), or (c) accept as a known wart. Note the error asymmetry: a **false** warning is safe, a **missed** one is not. ⚠ Also divergent: the natives enumerate devices and warn *"is not currently connected"*; the web only knows the selected input and warns *"is not the current input"* — wording was aligned 2026-07-19, but **condition parity rides this decision**. | *(this row)* |
