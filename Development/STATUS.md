...
_Last updated: 2026-08-01. **1.0.2 has shipped across all four surfaces** (Mac/iOS in final App Store
publish); the release-process tooling and the marketing-website extraction are complete (see Done).
...
**▶ Next up:** **9** align the frozen-recalc paired tests. No open item blocks 1.0.2.
...
- **Swift release-process finalization + 1.0.2 shipped** ✅ DONE (2026-08-01). Swift now mirrors Python's release-notes auto-stamping (build-less `Documentation/ReleaseNotes.md` + `generate-release-notes.sh` run from the macOS build) with a generated-docs gitignore clean slate; the User Manual HTML is **self-contained** (images embedded → older versions never drift). Marketing website extracted to its own repo `guitar_tap_website`; Swift `Website/` removed. **1.0.2 shipped across all 4 surfaces** — every repo committed + tagged `1.0.2` (Swift tag on the shipped build-455 commit), web app + site cPanel-deployed, Python GitHub Release created, App Store in final publish. Runbook: `GuitarTap/Documentation/RELEASE-CHECKLIST.md`. | *(no doc)*

- **Windows + Linux Python reference datasets captured** ✅ DONE (2026-07-30, user-captured) — the 1.0.2 pre-ship gate. Live guitar/plate/brace captures on Windows (`pc`) and Linux across mics, added to `GuitarTap/Tests/AllPlatforms/` beside the existing mac/iPad/iPhone/web set. | *(release-prep — no doc)*

- **Material measurement dimensions — override, display, sourcing (+ notes-on-load)** ✅ DONE all 3 (2026-07-30). Two-store model (Settings = template, per-measurement Store B edited in the Analysis Results); calc/display/PDF/save read Store B; Diagonal (fL/fC/fLC) naming + toggle rename; notes-on-load; §12 type derived from the snapshot; shared validated-number-field widget (3-way parity). Web `30dff21`/`b490c0a`/`228c24b`/`e11c6a4`/`ebc2f53`; toggle Swift `2233479`/Python `ead835c`; Chunk-D tests `581d04d` + widget `23e3cea`/`19d976e`. Docs: manual + Help + 1.0.2 release notes, all 3. | [MEASUREMENT-DIMENSIONS-SPEC.md](Completed/MEASUREMENT-DIMENSIONS-SPEC.md)
...
- **Python multi-tap material results one-phase lag** ✅ fixed + verified 2026-07-16 (resolve from the persistent per-phase lists, not transient `current_peaks`). | [PYTHON-MULTITAP-MATERIAL-RESULTS.md](Completed/PYTHON-MULTITAP-MATERIAL-RESULTS.md)

**Other completed this cycle:**

- **Export/save re-entrancy guard (all 3)** ✅ DONE + user-verified 2026-07-25. Repeated/double clicks stacked export panels. Swift: `isExportPanelInFlight` early-return on all 8 export/save entries. Web: per-button 700 ms debounce. Python: N/A (modal dialog). | *(no doc — this row)*
- **Soak / stress harness + the deinit crash it surfaced** ✅ DONE 2026-07-25. Scripts all 3; run-reviewed (web 200, Python 1000 on Windows, Swift ASan); the Swift soak root-caused + fixed the parked `TapToneAnalyzer.deinit` crash (fix = test target → `MainActor`). | [SOAK-STRESS-HARNESS.md](Completed/SOAK-STRESS-HARNESS.md) · [DEINIT-CRASH-INVESTIGATION.md](Completed/DEINIT-CRASH-INVESTIGATION.md)
...
- **Threshold meter reads high on web during playback** ✅ root-caused + fixed + user-verified 2026-07-18, committed `c732a04` (web-only, `@parity none`). **DEV-ONLY** — React StrictMode's dev double-mount raced the async `engine.start()` and leaked a 2nd live-mic engine (the web level math was exact all along); hardened `useAudioEngine.start()` to stop the orphaned engine. Production was never affected. | [MATERIAL-LIVE-ANNOTATION-DISPLAY.md](Completed/MATERIAL-LIVE-ANNOTATION-DISPLAY.md)
...
- **File paths & names audit → 1.0.2** ✅ all 8 steps committed 2026-07-15 (one commit per repo). Unified
  ...
  ([DECAY-AUDIO-CLOCK-FIX.md](Completed/DECAY-AUDIO-CLOCK-FIX.md)). Spec: [FILE-PATHS-AND-NAMES-SPEC.md](Completed/FILE-PATHS-AND-NAMES-SPEC.md).
...
| Doc | Purpose |
|---|---|
| [MEASUREMENT-DIMENSIONS-SPEC.md](Completed/MEASUREMENT-DIMENSIONS-SPEC.md) | material measurement dimensions — two-store model (Settings template vs per-measurement Store B, edited in the Analysis Results); Diagonal (fL/fC/fLC) naming + toggle rename; notes-on-load; §12 type-from-snapshot; shared validated-number-field widget (3-way parity) — ✅ done all 3 (2026-07-30) + docs (manual/Help/1.0.2 release notes) |
...
| [WEB-PDF-MATERIAL-LAYOUT.md](Completed/WEB-PDF-MATERIAL-LAYOUT.md) | web material PDF fixes A–L + the Swift/Python chart defects M–O (✅ done) |
| [PYTHON-MULTITAP-MATERIAL-RESULTS.md](Completed/PYTHON-MULTITAP-MATERIAL-RESULTS.md) | Python results one-phase-lag bug + fix (✅ done) |
| [ANALYSIS-QUALITY-MODECOLOR-CLEANUP.md](Completed/ANALYSIS-QUALITY-MODECOLOR-CLEANUP.md) | analysis-quality/mode-colour/quality-colour parity + tests (✅ done) |
| [MATERIAL-LIVE-ANNOTATION-DISPLAY.md](Completed/MATERIAL-LIVE-ANNOTATION-DISPLAY.md) | live material annotation display (item 12R) + the web threshold-meter finding — both ✅ done (the latter a dev-only StrictMode engine leak) |
| [FILE-PATHS-AND-NAMES-SPEC.md](Completed/FILE-PATHS-AND-NAMES-SPEC.md) | file-paths/names audit → 1.0.2 — ✅ all 8 steps committed |
| [DECAY-AUDIO-CLOCK-FIX.md](Completed/DECAY-AUDIO-CLOCK-FIX.md) | ring-out measured in audio time — ✅ done |
| [PLATFORM-PARITY-GAPS.md](Completed/PLATFORM-PARITY-GAPS.md) | Task 4 OUT-1…OUT-5 — ✅ all done |
| [OUT-4-DETECTION-SPEC.md](Completed/OUT-4-DETECTION-SPEC.md) | OUT-4 spec + noisy-fixture design — ✅ done |
| [PHASE6-PARITY.md](Completed/PHASE6-PARITY.md) | Phase 6 parity backlog — ✅ complete |
| [6-TEST-NORMALIZATION.md](Completed/6-TEST-NORMALIZATION.md) | 6-TEST normalization — ✅ complete (Phases 1–6) |
| [LOG-FREQ-REMOVAL.md](Completed/LOG-FREQ-REMOVAL.md) | removed the dead log-frequency axis — ✅ done |
| [TAPTONEANALYZER-CONSOLIDATION.md](Completed/TAPTONEANALYZER-CONSOLIDATION.md) | Phase 6-3c analyzer/device split — ✅ done |
| [STATUS-STATE-MACHINE.md](Completed/STATUS-STATE-MACHINE.md) | status state-machine alignment — ✅ done |
| [REVIEW-TRACKER.md](Completed/REVIEW-TRACKER.md) | Task 1/2 doc-review tracker (done) + spun-out effort specs |
| [REVIEW-FINDINGS.md](Completed/REVIEW-FINDINGS.md) | behavioural gaps found during the doc review |
| [PHASE2-DSP-HARNESS.md](Completed/PHASE2-DSP-HARNESS.md) · [PHASE3-UI.md](Completed/PHASE3-UI.md) · [PHASE4-PERSISTENCE.md](Completed/PHASE4-PERSISTENCE.md) | phase specs — done |
