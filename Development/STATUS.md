...
**▶ Next up (2026-07-25):** **9** — verify the Python progress-bar-lingers reproduces (then fix or close) · **11** (release-prep) — user captures Windows + Linux Python reference datasets before shipping 1.0.2. Item 10 (export/save re-entrancy guard) ✅ done + user-verified all 3. Per the pre-release review, **no open item blocks 1.0.2**.
...
- **Export/save re-entrancy guard (all 3)** ✅ DONE + user-verified 2026-07-25. Export/save present an async, non-modal panel, so repeated / double clicks stacked exports. **Swift:** `isExportPanelInFlight` early-return on every export/save entry (main view ×2 + `MeasurementsListView` ×6); the in-flight flags already cleared on OK/cancel/error, so the guard can't wedge a button. **Web:** per-button **700 ms timestamp debounce** — on the download-fallback path (no `showSaveFilePicker`) the export finishes instantly, so a `useState` flag / in-flight ref clears before the 2nd click of a double-click; kept `isExportingRef` for save-picker overlap. **Python:** N/A — modal `QFileDialog`/`exec()` blocks re-entry by construction. | *(no doc — this row)*
- **Soak / stress harness + the deinit-crash it surfaced** ✅ DONE 2026-07-25. Cross-platform dev-tool soak scripts committed all 3 (`Tooling/soak.sh` web + Python, portable bash; `Tooling/deinit-soak.sh` Swift + a live per-run tally). Run-reviewed: web 200/200, **Python 1000 on Windows** (+ overnight live) — the platform where the reported QObject-GC crash lived — both clean. The Swift ASan soak reproduced and then **root-caused + fixed** the parked `TapToneAnalyzer.deinit` teardown crash: it was off-main teardown of the `@MainActor` analyzer's Combine `Published.Storage` under swift-testing parallelism (production releases on main via SwiftUI, so was never exposed); fix = default the **test** target to `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor` (no production code change), ASan-clean 93/100 where it tripped by run 5–8. | [SOAK-STRESS-HARNESS.md](Completed/SOAK-STRESS-HARNESS.md) · [DEINIT-CRASH-INVESTIGATION.md](Completed/DEINIT-CRASH-INVESTIGATION.md)
