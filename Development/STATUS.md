...
| 10 | **Peak lifecycle — durability of per-peak state** | 📋 Spec agreed + Swift gap analysis + 8-phase plan; **no Swift code yet**. One root cause behind a family of bugs (deselect not sticking, dragged labels destroyed, state leaking between measurements, peaks re-analysed on pan/zoom): a *display* change re-runs detection and discards peak identity. Swift → Python → web. Interim Python pan/zoom fix already written (uncommitted, awaiting run-review). | [PEAK-LIFECYCLE-PLAN-SWIFT.md](PEAK-LIFECYCLE-PLAN-SWIFT.md) |
...
| [PEAK-LIFECYCLE-PLAN-SWIFT.md](PEAK-LIFECYCLE-PLAN-SWIFT.md) | Peak lifecycle | 8-phase sequenced implementation plan for Swift |
