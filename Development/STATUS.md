...
|---|---|---|---|
| 1 | **Task 3 / 6-TEST** — cross-platform test-suite normalization (major) | **In progress.** Phases 1-2 done; Phase-3 PC-1..4 done (2026-07-09); **3c consolidation ✅ complete 2026-07-12** (web analyzer/device now mirror canonical `TapToneAnalyzer`/`RealtimeFFTAnalyzer`); EG-1/EG-2/EG-3 + frozen-peak-recalc (engine half) + import-persistence — all 3-way. `annotation-state` is entirely the selection/override/offset subsystem → **rescheduled under P3** (it's view-side today; testing it now would be thrown away when P3 moves it to the analyzer). PC-1 docs ✅ — **Phase 3 complete**. **✅ Phase 4 COMPLETE** — all 4 test orphans resolved (`gen_parity_map.py --check` → 0 problems): 4a (guitar-fft + gated-capture were wrong-altitude DSP shortcuts → deleted, slugs retired); 4b (status-message 3-way — new Swift `StatusMessageTests.swift` + Python `test_status_message.py`); 4c (tap-count-change 3-way — new Swift `TapCountChangeTests.swift` + Python `test_tap_count_change.py` + a web model `describe`; **no production change needed** — Python's `set_tap_num` already mirrored Swift's `didSet`). Excluded/deferred as tracked: OUT-1 phase-guidance strings, the pipeline-progress transients (4b follow-up), OUT-5 (reduce-count reduction path). **Phase 5 ✅** — `tests=` populated on the testable impl groups (10 newly linked); new coverage-gap report in `gen_parity_map.py` (`--gaps`) shows the 2 honest gaps (dsp/analysis-quality, model/mode-colors); Swift gesture + Python Qt suites tagged `@parity none`; folding + oracle-vs-goldens documented. `--check` = 61 groups, 0 orphans, 2 platform-specific. **NEXT: Phase 6** (finalize the matrix as the living coverage doc + wire `--check` + the coverage-gap report as CI gates). **P3** (item 2 restructure) lands frozen-peak-recalc's selection half + annotation-state 3-way. | [6-TEST-NORMALIZATION.md](6-TEST-NORMALIZATION.md) |
...
| 4 | **Platform parity gaps** (Swift/Python/web behavior + UI) | Backlog — **4 open** (OUT-2/3 from the 6-3c review; **OUT-4** material tap-detection model — relative noise-floor EMA vs absolute dBFS; **OUT-5** reduce-count-mid-sequence — Swift defers vs Python synchronous finalise). **OUT-1 ✅ FIXED + user run-reviewed** — the status state-machine alignment made the Swift/Python warm-up silent (status derived-from-state, like the web), so phase-guidance is now visible; run-review fixes rode along (Python status bar single-sourced from `analyzer.status_message`; web material-L arm → "Ready for L tap"). A **separate cross-platform effort, after Phase 6**; OUT-2/3/4/5 not started. | [PLATFORM-PARITY-GAPS.md](PLATFORM-PARITY-GAPS.md) · [STATUS-STATE-MACHINE.md](STATUS-STATE-MACHINE.md) |
