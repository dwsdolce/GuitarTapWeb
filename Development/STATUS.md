...
|---|---|---|---|
| 1 | **Task 3 / 6-TEST** — cross-platform test-suite normalization (major) | **In progress.** Phases 1-2 done; Phase-3 PC-1..4 done (2026-07-09); **3c consolidation ✅ complete 2026-07-12** (web analyzer/device now mirror canonical `TapToneAnalyzer`/`RealtimeFFTAnalyzer`); EG-1/EG-2/EG-3 + frozen-peak-recalc (engine half) + import-persistence — all 3-way. **NEXT: pure-gap suite** (guitar annotation-state) → orphan test back-ports → PC-1 docs → P3 (also completes frozen-peak-recalc's selection/annotation half). | [6-TEST-NORMALIZATION.md](6-TEST-NORMALIZATION.md) |
