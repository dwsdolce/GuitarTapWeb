...
| 9 | **Threshold input-level meter reads high on web** vs Swift/Python | 📋 OPEN, found 2026-07-17 during the plate-playback testing. Same file → the web's Threshold meter shows a much higher input level than the natives. Detection still succeeds on all three (final peaks correct) → a level-**display** scaling difference (calibration/dBFS reference on the meter), NOT a detection defect and NOT caused by the respin. Needs its own investigation; possibly the same family as the buffer-size item but distinct. | [MATERIAL-LIVE-ANNOTATION-DISPLAY.md](MATERIAL-LIVE-ANNOTATION-DISPLAY.md) §Still-open |
