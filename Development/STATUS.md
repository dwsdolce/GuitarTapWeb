...
| 10 | **Peak lifecycle — durability of per-peak state** | 🟡 In progress. Swift ALL phases + docs DONE (committed). **Python port: Phases 1–5 DONE + user-verified** (1+2 `6068d1a`, 3 `a3c5f02`, 4 `6b6cc7d`, 4a `b406690`, 5 pending-commit; suite 544, parity 79). **RESUME at Phase 6** (derived values read the definitive peak). Web after Python. A `TapToneAnalyzer.deinit` crash under swift-testing **parallelism** is PARKED (cross-suite teardown race; not a production path). Also see the Python↔Swift frozen-recalc test-fixture divergence (item 14). | [SWIFT plan](PEAK-LIFECYCLE-PLAN-SWIFT.md) · [PYTHON plan](PEAK-LIFECYCLE-PLAN-PYTHON.md) · [deinit crash](DEINIT-CRASH-INVESTIGATION.md) |
...
| 13 | **Swift export/save actions have no re-entrancy guard** | 📋 Open, hazard not a reported bug — noted 2026-07-22 while investigating a *false alarm* (an apparently hung PDF export that turned out to be a save panel hidden behind the window; no defect). `exportPDFReport` / `exportMultiTapPDFReport` / `exportComparisonPDFReport` set `isGeneratingReport` whose ONLY reset is `NSSavePanel.begin`'s completion, with no `.disabled(…)` on the buttons and an overlay that cannot paint in time (`ImageRenderer` runs synchronously on the main thread). Repeated clicks stack panels. Same pattern for `isExportingSpectrum` / `isSavingMeasurement`, and `MeasurementDetailView` / `MeasurementsListView` hold their own copies. Fix = early-return when already in flight + overlay blocks hit-testing. Check Python/web for the same shape. | *(this row)* |
| 14 | **Python↔Swift test-fixture divergence (frozen-recalc)** | 📋 Open, found 2026-07-23 during the Phase 4a port. Python's frozen-recalc tests drive **real `find_peaks` detection** (`_make_spectrum_with_peak`); Swift **injects** pre-computed peaks (`freeze`). Same `test/frozen-peak-recalc` slug, different approach — so the paired tests aren't true twins, and `--check` a week ago passed anyway (it verifies slug presence, not equivalence). Audit Python's frozen-recalc class against Swift's `FrozenPeakRecalculationTests` and align fixture-for-fixture. A `--check` gap: slug pairing ≠ behavioural equivalence. | [FROZEN-RECALC-TEST-PARITY.md](FROZEN-RECALC-TEST-PARITY.md) |
