...
| 10 | **Python re-IDs peaks on every pan/zoom** | 📋 Open. `_refresh_peaks_for_viewport` calls `recalculate_frozen_peaks_if_needed()` on every pan/zoom, churning peak UUIDs and re-matching offsets/overrides/selection by frequency; Swift/web just re-filter. Item 3 of the doc. | [DOT-ANNOTATION-PARITY.md](DOT-ANNOTATION-PARITY.md) |
...
| [DOT-ANNOTATION-PARITY.md](DOT-ANNOTATION-PARITY.md) | Dot vs annotation lists | items 1–2 ✅ (web dots on `isKnown`; `view/dot-layer` group, all 3); item 3 open — Python re-IDs peaks on pan/zoom |
