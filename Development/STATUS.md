...
| 8 | **Missing-test parity slugs** (measurementTypeName + analysis-quality + mode-colors + quality-colors) | ✅ **DONE + committed 2026-07-18.** `measurementTypeShortName` extracted (non-private) + tested; the 3 untested `@parity` slugs now have tests on all 3; Python analysis-quality consolidated to one source (`extensions.py`); Swift annotation color de-duped to `GuitarMode.color`. `--check` fully clean (74 groups). Color-on-model layer violation stays deferred to item 3 (styles). | [ANALYSIS-QUALITY-MODECOLOR-CLEANUP.md](ANALYSIS-QUALITY-MODECOLOR-CLEANUP.md) · MATERIAL-MULTITAP §2 |
...
| 12 | **Swift/Python non-web defects (index)** | ✅ **M/N/R DONE + user-verified 2026-07-17** (respin Step 2): M plate chips role labels; N legend `(L)`/`(C)` suffixes; R live material annotations now show the accumulated identified peaks on both natives (progressive, no flicker). ✅ **O (date format) DONE + committed 2026-07-18** — CLDR `atTime` glue: Swift `DateDisplay` + web `format/date` now format date+time in SEPARATE calls joined with `, ` (both the list `string(_:)` and the compact comparison-legend variant); Python already comma. 📋 **P/Q (panel spacing · Gore nesting) still open, post-respin.** | [WEB-PDF-MATERIAL-LAYOUT.md](WEB-PDF-MATERIAL-LAYOUT.md) §M–O · [MATERIAL-LIVE-ANNOTATION-DISPLAY.md](MATERIAL-LIVE-ANNOTATION-DISPLAY.md) (R) · [RESULTS-PANEL-CONSISTENCY.md](RESULTS-PANEL-CONSISTENCY.md) §3,§7 |
