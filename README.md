...
- **[ESLint]** with `eslint-plugin-react-hooks` — a focused lint for React Hook
  dependency arrays (types/unused are covered by the strict TypeScript check).
...
npm run lint         # ESLint — React Hook dependency-array checks
```
...
`npm test`, `npm run typecheck`, and `npm run lint`. Because this is a parity port, changes to DSP
